...
class WerewolfPowersForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        breed_gift_choices = [
            (x.pk, x.name) for x in Gift.objects.filter(rank=1).order_by('name') if self.char.breed in x.allowed["garou"]
        ]
        auspice_gift_choices = [
            (x.pk, x.name) for x in Gift.objects.filter(rank=1).order_by('name') if self.char.auspice in x.allowed["garou"]
        ]
        tribe_gift_choices = [
            (x.pk, x.name) for x in Gift.objects.filter(rank=1).order_by('name') if self.char.tribe.name in x.allowed["garou"]
        ]
        super().__init__(*args, **kwargs)
